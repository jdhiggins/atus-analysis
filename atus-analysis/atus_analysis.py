...
def replace_code(code):
    for number in atus_codes_list:
        if number[0][1:] == str(code):
            return number[1]
...
def make_df():
    ...
    cps_small = cps.loc[:,['TUCASEID', 'TULINENO', 'GESTFIPS',
                           'HEFAMINC', 'HETENURE', 'PREMPHRS',
                           'PRMJIND1']]
    cps_data = cps_small[cps_small['TULINENO'] == 1]
    cps_data.pop('TULINENO')
    merged = pd.merge(cps_data, summary, left_on="TUCASEID",
                      right_on="tucaseid")
    return merged


state_codes = {1: 'AL', 2: 'AK', 4: 'AZ', 5: 'AR', 6: 'CA',
...
def make_averages_by_state(frame, code):
    new_data = frame.loc[:, ['TUFINLWGT', code, 'GESTFIPS']]
    new_data['weighted_minutes'] = new_data.TUFINLWGT * new_data[code]
    new_data = new_data.groupby('GESTFIPS').sum()
    new_data['average_weighted_minutes'] = new_data["weighted_minutes"] / new_data["TUFINLWGT"]
    new_data.index = new_data.index.to_series().map(lambda fips: replace_fips(fips))
    new_data = new_data['average_weighted_minutes']
    new_data = new_data.order(ascending=False)
    return new_data[0:10]